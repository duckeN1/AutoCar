        ...
        try:
            ...
            while True:
                ...
                for res in results:
                    ...
                    tocka1 = (int)((res[1][0] + res[1][2]) / 2)
                    tocka2 = (int)((res[1][1] + res[1][3]) / 2)
                    points.append((tocka1, tocka2))

                    cv2.rectangle(self.raw_image, (tocka1, tocka2), (tocka1 + 1, tocka2 + 1), (0, 255, 0), 2)


                if(len(points) >= 3):
                    hull = cv2.convexHull(np.array(points), returnPoints=True)

                    for h in range(hull.shape[0] - 1):
                        t1 = np.array(hull[h])
                        t2 = np.array(hull[h + 1])
                        cv2.line(self.raw_image, (t1[0][0], t1[0][1]), (t2[0][0], t2[0][1]),[0,255,0],2)
